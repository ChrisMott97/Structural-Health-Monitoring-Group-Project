...
The model sensitivies affect both the time series and the autoencoder models and can be different to your own profile sensitivity settings. Sensors are added by ID and are grouped by location during selection. To add all sensors to the report, click the 'Add All Sensors' button and to remove a sensor from the list, click on its ID in the section below. However, please be aware this may impact performance when a very large amount of sensors are being used.
